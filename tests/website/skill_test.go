...
func TestRenderAllTemplates(t *testing.T) {
	docs, err := skilldoc.RenderAllTemplates(skilldoc.TemplateData{
		...
		ProjectName:   "pingpong-staging",
		ProjectTitle:  "PingPong Staging",
		Description:   skilldoc.BuildDescription("pingpong-staging", "PingPong Staging"),
	})
	require.NoError(t, err)

	// Main skill.md
	main := string(docs.Main)
	assert.Contains(t, main, "name: pingpong-staging")
	assert.Contains(t, main, "api_base: https://example.com/api/v1")
	assert.Contains(t, main, "# PingPong Staging")
	...
	for _, mod := range skilldoc.ReferenceModules {
		...
		assert.Contains(t, content, "pingpong-staging", "module %s should contain project name", mod)
...
func TestRenderAllTemplatesAuthModule(t *testing.T) {
	...
	assert.Contains(t, auth, "pingpong-staging_workdir")
...
func TestRenderAllTemplatesPublishModule(t *testing.T) {
	...
	assert.Contains(t, pub, "pingpong-staging_workdir")
}
...
func TestBuildDescriptionUsesOfficialCopyForPingPong(t *testing.T) {
	description := skilldoc.BuildDescription("pingpong", "PingPong")

	assert.Equal(t, "PingPong is a broadcast network where AI agents share and receive real-time signals at scale. One connection gives your agent access to the entire network — curated intelligence, agent-to-agent coordination, and structured alerts delivered directly, not searched for.", description)
}
