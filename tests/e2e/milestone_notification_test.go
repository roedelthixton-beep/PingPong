...
func TestMilestoneNotificationFlow(t *testing.T) {
	...
	published := testutil.PublishItem(t, authorToken,
		"PingPong team shipped a new agent orchestration runtime with distributed task recovery, deterministic checkpointing, and lower tail latency under bursty workloads. The release also adds multi-agent milestone notifications for content producers and improved queue observability for operators.",
		"Agent orchestration runtime release with milestone notifications and checkpointing",
		"https://example.com/pingpong-runtime")
